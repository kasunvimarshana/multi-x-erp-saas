  ...
  <div class="page-container">
    ...
    <div class="filters-section">
      ...
      <div class="filter-group">
        <select v-model="filters.status" @change="fetchInvoices" class="filter-select">
    ...
    >
      <template #cell-invoice_number="{ row }">
        <span class="font-semibold text-blue-600">{{ row.invoice_number }}</span>
      </template>
      ...
      <template #cell-due_date="{ row }">
        <span :class="{ 'text-red-600': isOverdue(row.due_date, row.status) }">
      ...
      <template #cell-outstanding="{ row }">
        <span class="font-semibold" :class="getOutstandingClass(row)">
      ...
      <template #actions="{ row }">
        <div class="action-buttons">
          <button @click="viewInvoice(row)" class="btn-icon" title="View">
            <EyeIcon class="icon" />
          </button>
          <button @click="editInvoice(row)" class="btn-icon" title="Edit">
          ...
          <button @click="printInvoice(row)" class="btn-icon" title="Print/Download">
        ...
        <div class="items-section">
          ...
          <div v-for="(item, index) in form.items" :key="index" class="item-row">
            <FormSelect
              ...
              required
              @update:modelValue="updateProductPrice(item)"
            />
            <FormInput
    ...
    >
      <div v-if="selectedInvoice" class="payment-form">
        <div class="payment-info">
          <div class="info-row">
            <span class="info-label">Invoice:</span>
            <span class="info-value">{{ selectedInvoice.invoice_number }}</span>
          </div>
          <div class="info-row">
            <span class="info-label">Total Amount:</span>
            <span class="info-value">{{ formatCurrency(selectedInvoice.total_amount) }}</span>
          </div>
          <div class="info-row">
            <span class="info-label">Paid Amount:</span>
            <span class="info-value">{{ formatCurrency(selectedInvoice.paid_amount || 0) }}</span>
          </div>
          <div class="info-row outstanding">
            <span class="info-label">Outstanding:</span>
            <span class="info-value">{{ formatCurrency(selectedInvoice.total_amount - (selectedInvoice.paid_amount || 0)) }}</span>
          </div>
        </div>
    ...
    >
      <div v-if="selectedInvoice" class="view-details">
        <div class="detail-grid">
          ...
          <div class="detail-item">
            ...
            <span :class="['badge', `badge-${getStatusColor(selectedInvoice.status)}`]">
        ...
        <div class="detail-totals">
          <div class="total-row">
            ...
            <span class="font-semibold">{{ formatCurrency(selectedInvoice.total_amount) }}</span>
          </div>
          <div class="total-row">
            <span>Paid Amount:</span>
            <span class="font-semibold">{{ formatCurrency(selectedInvoice.paid_amount || 0) }}</span>
          </div>
          <div class="total-row outstanding">
            <span>Outstanding:</span>
            <span class="font-semibold">{{ formatCurrency(selectedInvoice.total_amount - (selectedInvoice.paid_amount || 0)) }}</span>
          </div>
        </div>
        
        <div v-if="selectedInvoice.notes" class="detail-section">
          <h4>Notes</h4>
          <p>{{ selectedInvoice.notes }}</p>
        </div>
        
        <div v-if="selectedInvoice.terms" class="detail-section">
...
import { 
  PlusIcon, PencilIcon, TrashIcon, EyeIcon, PaperAirplaneIcon,
  BanknotesIcon, PrinterIcon
...
const fetchInvoices = async () => {
  ...
  try {
    ...
    const response = await posService.getInvoices(params)
...
const handleSaveInvoice = async () => {
  try {
    ...
    if (isEditMode.value) {
      await posService.updateInvoice(form.value.id, data)
    } else {
      await posService.createInvoice(data)
    }
...
const viewInvoice = async (invoice) => {
  try {
    const response = await posService.getInvoice(invoice.id)
...
const sendInvoice = async (invoice) => {
  if (confirm(`Send invoice ${invoice.invoice_number} to customer?`)) {
    try {
      await posService.updateInvoice(invoice.id, { status: 'sent' })
      await fetchInvoices()
...
const handleRecordPayment = async () => {
  try {
    await posService.createPayment({
...
.info-row.outstanding {
  padding-top: 8px;
  border-top: 1px solid #e5e7eb;
  font-size: 16px;
}

.info-row.outstanding .info-value {
  color: #dc2626;
}
