<template>
  <div class="page-container">
    <div class="page-header">
      <h1 class="page-title">StockMovements</h1>
    ...
    <Modal
      ...
      title="Add StockMovement"
      @close="showCreateModal = false"
      @confirm="handleCreate"
    >
      <p class="text-muted">Form fields for StockMovement will appear here</p>
    </Modal>
  </div>
</template>
...
const stock_movements = computed(() => store.stock_movements)
...
const fetchItems = async () => {
  await store.fetchStockMovements({ page: currentPage.value })
}
...
const deleteItemConfirm = async (item) => {
  if (confirm('Are you sure you want to delete this item?')) {
    // await store.deleteStockMovement(item.id)
  }
}
</script>
